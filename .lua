...
-- FEATURES: FLY, NOCLIP, FULLBRIGHT, PLAYER TP LIST, WAYPOINTS
...
local function MobileBtn(txt, pos, size, color, parent)
    ...
    b.Font = "SourceSansBold"; b.TextSize = 14
...
FFrame.Size = UDim2.new(0, 200, 0, 110); FFrame.Position = UDim2.new(0.05, 0, 0.4, 0); FFrame.BackgroundColor3 = Color3.fromRGB(20, 20, 20); FFrame.Active = true; Drag(FFrame)
Instance.new("UICorner", FFrame); local stroke = Instance.new("UIStroke", FFrame); stroke.Color = Color3.fromRGB(0, 170, 255); stroke.Thickness = 2
...
speedShow.Text = "SPD: 50"; speedShow.Size = UDim2.new(0.3, 0, 0, 30); speedShow.Position = UDim2.new(0.65, 0, 0.25, 0); speedShow.TextColor3 = Color3.fromRGB(0, 170, 255); speedShow.BackgroundTransparency = 1; speedShow.Font = "SourceSansBold"
...
local closeF = MobileBtn("X", UDim2.new(0.85, 0, -0.2, 0), UDim2.new(0, 25, 0, 25), Color3.fromRGB(200, 0, 0), FFrame)
        ...
        task.spawn(function()
            while isFlying and char and char:FindFirstChild("Humanoid") do
                ...
                if moveDir.Magnitude > 0 then
                    ...
                    local finalDir = (camLook * moveDir.Z * -1) + (camRight * moveDir.X)
...
MFrame.Size = UDim2.new(0, 260, 0, 330); MFrame.Position = UDim2.new(0.5, -130, 0.5, -165); MFrame.BackgroundColor3 = Color3.fromRGB(15, 15, 15); MFrame.Active = true; Instance.new("UICorner", MFrame); Drag(MFrame)
...
-- --- 4. PLAYER LIST PANEL (SMALLER) ---
local listFrame = Instance.new("Frame", neonGui)
listFrame.Size = UDim2.new(0, 160, 0, 230); listFrame.Position = UDim2.new(0.5, 140, 0.5, -115); listFrame.BackgroundColor3 = Color3.fromRGB(15, 15, 15); listFrame.Visible = false; listFrame.Active = true; Instance.new("UICorner", listFrame); Drag(listFrame)
Instance.new("UIStroke", listFrame).Color = Color3.fromRGB(0, 170, 255)
local listScroll = Instance.new("ScrollingFrame", listFrame); listScroll.Size = UDim2.new(1, -10, 1, -40); listScroll.Position = UDim2.new(0, 5, 0, 10); listScroll.BackgroundTransparency = 1; listScroll.ScrollBarThickness = 2

local function updatePlayerList()
    for _, child in pairs(listScroll:GetChildren()) do child:Destroy() end
    local count = 0
    for _, p in pairs(Players:GetPlayers()) do
        if p ~= lp then
            local pBtn = MobileBtn(p.DisplayName, UDim2.new(0, 0, 0, count * 35), UDim2.new(1, 0, 0, 30), Color3.fromRGB(30, 30, 30), listScroll)
            pBtn.Activated:Connect(function() if p.Character then lp.Character.HumanoidRootPart.CFrame = p.Character.HumanoidRootPart.CFrame end end)
            count = count + 1
        end
    end
...
-- --- 5. WAYPOINTS PANEL (SMALLER) ---
local waypoints = {}
local posFrame = Instance.new("Frame", neonGui)
posFrame.Size = UDim2.new(0, 160, 0, 230); posFrame.Position = UDim2.new(0.5, -310, 0.5, -115); posFrame.BackgroundColor3 = Color3.fromRGB(15, 15, 15); posFrame.Visible = false; posFrame.Active = true; Instance.new("UICorner", posFrame); Drag(posFrame)
Instance.new("UIStroke", posFrame).Color = Color3.fromRGB(0, 170, 255)
local posScroll = Instance.new("ScrollingFrame", posFrame); posScroll.Size = UDim2.new(1, -10, 1, -85); posScroll.Position = UDim2.new(0, 5, 0, 10); posScroll.BackgroundTransparency = 1; posScroll.ScrollBarThickness = 2
local posInput = Instance.new("TextBox", posFrame); posInput.Size = UDim2.new(0.9, 0, 0, 30); posInput.Position = UDim2.new(0.05, 0, 1, -75); posInput.BackgroundColor3 = Color3.fromRGB(25, 25, 25); posInput.TextColor3 = Color3.new(1,1,1); posInput.PlaceholderText = "Name..."; posInput.Font = "SourceSansBold"; posInput.TextSize = 12; Instance.new("UICorner", posInput)

local function updatePosList()
    for _, child in pairs(posScroll:GetChildren()) do child:Destroy() end
    local count = 0
    for name, cf in pairs(waypoints) do
        ...
        pBtn.Activated:Connect(function() if lp.Character then lp.Character.HumanoidRootPart.CFrame = cf end end)
        count = count + 1
    end
...
HubBtn("Noclip: OFF", MainTab, 55, function(b) 
    _G.noclip = not _G.noclip; b.Text = _G.noclip and "Noclip: ON" or "Noclip: OFF"
    b.TextColor3 = _G.noclip and Color3.new(0,1,0) or Color3.new(1,1,1)
end)
HubBtn("Fullbright: OFF", MainTab, 100, function(b) 
    _G.fb = not _G.fb; b.Text = _G.fb and "Fullbright: ON" or "Fullbright: OFF"
    b.TextColor3 = _G.fb and Color3.new(0,1,0) or Color3.new(1,1,1)
end)
HubBtn("Teleport Menu", MainTab, 145, function() listFrame.Visible = not listFrame.Visible; if listFrame.Visible then updatePlayerList() end end)
HubBtn("Saved Positions", MainTab, 190, function() posFrame.Visible = not posFrame.Visible; if posFrame.Visible then updatePosList() end end)

-- Inside Pos Frame Button
...
saveBtn.Activated:Connect(function()
    local name = posInput.Text ~= "" and posInput.Text or "Pos "..(#waypoints + 1)
    if lp.Character then waypoints[name] = lp.Character.HumanoidRootPart.CFrame; posInput.Text = ""; updatePosList() end
end)
...
UserInputService.InputBegan:Connect(function(input, chat)
    if not chat and input.KeyCode == Enum.KeyCode.K then MFrame.Visible = not MFrame.Visible end
end)

-- --- 8. LOOPS & NOTIFY ---
RunService.RenderStepped:Connect(function()
    if _G.fb then Lighting.Ambient = Color3.new(1,1,1); Lighting.OutdoorAmbient = Color3.new(1,1,1); Lighting.Brightness = 2; Lighting.ClockTime = 14 end
end)
...
game:GetService("StarterGui"):SetCore("SendNotification", {Title = "Neon Hub", Text = "K to Toggle (PC) | H Button (Mobile)", Duration = 5})
